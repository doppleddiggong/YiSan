...
void APlayerActor::DelayedSendQuestUpdate()
{
	...
	{
		if (const AYisanGameState* GameState = World->GetGameState<AYisanGameState>())
		{
			if (const AQuestManagerActor* QuestManager = GameState->GetQuestManager())
			{
				BroadcastManager->SendUpdateQuest(QuestManager->GetCurrentQuestTarget());
			}
		}
	}
}

void APlayerActor::ReportQuestContact(EBuildingType BuildingType)
{
	if (AQuestManagerActor* QuestManager = AQuestManagerActor::Get(this))
	{
		QuestManager->ServerRPC_NotifyContact(BuildingType);
	}
}
