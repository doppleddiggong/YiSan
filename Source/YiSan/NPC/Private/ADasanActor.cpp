	...
	if (HasAuthority())
	{
		...
		else
		{
			...
			DasanAicontrol->ReceiveMoveCompleted.AddDynamic(this, &ADasanActor::OnMoveCompleted);
				...
				if (AYisanGameState* GameState = GetWorld()->GetGameState<AYisanGameState>())
				{
						if (const AQuestManagerActor* QuestManagerDebug = GameState->GetQuestManager())
						{
								PRINTLOG(TEXT(" GameState quest target ready: %s"), *ENUM_TO_NAME(EBuildingType, QuestManagerDebug->GetCurrentQuestTarget()));
						}
						else
						{
								PRINTLOG(TEXT(" QuestManager not found on server"));
						}
				}
				else
				{
						PRINTLOG(TEXT(" GameState not found on server"));
				}
...
void ADasanActor::StartTour()
{
	...
	AYisanGameState* GameState = GetWorld()->GetGameState<AYisanGameState>();
	if (!GameState)
	{
		PRINTLOG(TEXT(" GameState를 찾을 수 없음"));
		return;
	}

	const AQuestManagerActor* QuestManager = GameState->GetQuestManager();
	if (!QuestManager)
	{
		PRINTLOG(TEXT(" QuestManager를 찾을 수 없음"));
		return;
	}

	const EBuildingType TargetType = QuestManager->GetCurrentQuestTarget();
	PRINTLOG(TEXT(" 다산 캐릭터의 추적 건물: %s"), *FString(ENUM_TO_NAME(EBuildingType, TargetType)));
...
{
		const AYisanGameState* GameState = GetWorld()->GetGameState<AYisanGameState>();
		if (!GameState)
		{
				return CurTargetBuilding; // 기존 타깃 유지
		}

		const AQuestManagerActor* QuestManagerLocal = GameState->GetQuestManager();
		if (!QuestManagerLocal)
		{
				return CurTargetBuilding;
		}

		const EBuildingType TargetType = QuestManagerLocal->GetCurrentQuestTarget();
	...
	for (auto Building : FoundBuildings)
	{
		...
		PRINTLOG(TEXT("[WARN] FindCurTargetBuilding: 타겟 건물을 찾을 수 없음 (%s)"), *FString(ENUM_TO_NAME(EBuildingType, TargetType)));
...
void ADasanActor::NextQuest()
{
	...
	const AYisanGameState* GameState = World->GetGameState<AYisanGameState>();
	if (!GameState)
		return;
	...
	// 퀘스트 정보가 없으면 이동 중단
	if (!QuestManager->HasActiveQuest() || TargetType == EBuildingType::None)
	{
