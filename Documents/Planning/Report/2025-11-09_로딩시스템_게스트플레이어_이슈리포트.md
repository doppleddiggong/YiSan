...
### 6.1 수정 파일
- `Source/YiSan/Character/Private/UChatPlayerSystem.cpp` (최종 해결)
- `Source/YiSan/Loading/Private/UYiSanLoading.cpp` (시도했으나 작동 안함)
- `Source/YiSan/Character/Private/APlayerControl.cpp` (시도했으나 작동 안함)

### 6.2 최종 수정 위치
- 파일: `UChatPlayerSystem.cpp`
- 함수: `UChatPlayerSystem::AnnouncePlayerJoin()` (Line 100)
...
void UChatPlayerSystem::AnnouncePlayerJoin()
{
	...
	const FString Message = FString::Printf(TEXT("%s 전하 어가, 행궁에 이르셨습니다."),
		*Owner->GetPlayerDisplayName());
...
#### 시도 2: ForceCompleteForGuest 함수 추가
...
**실패 원인**: `HandleLoadingComplete()` 자체가 호출되지 않음
...
1. **플레이어 입장 시점 감지**
   - "행궁에 이르셨습니다" 메시지는 플레이어가 완전히 준비되었을 때만 표시됨
   - 이 시점은 Pawn이 스폰되고, 이름이 설정되고, UI가 초기화된 시점
   ```cpp
   void UChatPlayerSystem::AnnouncePlayerJoin()  // UPlayerHeadWidget에서 호출됨
   ```
...
2. **직접 로딩 화면 숨김**
   ...
   if (Owner)
   {
       if (auto PC = Cast<APlayerControl>(Owner->GetController()))
       {
           if (PC->IsLocalController())  // 로컬 플레이어만
           {
               if (auto TM = ULoadingTransitionManager::Get(PC))
               {
                   TM->HideLoadingScreen();  // ← 직접 호출!
               }
           }
       }
   }
...
3. **장점**
   - ✅ 복잡한 로딩 시스템 델리게이트 흐름과 독립적
   - ✅ 실제로 플레이 가능한 시점에 로딩 숨김
   - ✅ 호스트/게스트 모두 동일한 로직으로 처리
   - ✅ 간단하고 명확한 코드

### 6.6 문제 해결 과정

#### 첫 번째 시도 (실패) - PostLoadMapWithWorld에서 Broadcast_HideLoading 호출
**수정 위치**: `UYiSanLoading.cpp:165`
```cpp
if (bIsFreshJoin && InWorld->GetNetMode() == NM_Client)
{
    CompleteState[EState::COMPLETE] = true;
...
```
**결과**: ❌ 작동하지 않음
**원인**: 게스트가 이미 로드된 맵에 join하면 `PostLoadMapWithWorld` 델리게이트 자체가 호출되지 않음
- 델리게이트 등록은 `PrepareForTravel()`에서 이루어짐
- 게스트는 `PrepareForTravel()`을 호출하지 않음
- 따라서 델리게이트가 등록되지 않아 `PostLoadMapWithWorld`가 실행되지 않음

#### 두 번째 시도 (실패) - ForceCompleteForGuest 함수 추가
**수정 위치**: `UYiSanLoading.cpp:481`, `APlayerControl.cpp:444`
```cpp
// UYiSanLoading에 강제 완료 함수 추가
void UYiSanLoading::ForceCompleteForGuest()
{
    if (TotalTime < KINDA_SMALL_NUMBER && World->GetNetMode() == NM_Client)
    {
...
```
**결과**: ❌ 작동하지 않음
**원인**: `HandleLoadingComplete()` 자체가 호출되지 않음
- 서버는 이미 로딩을 완료한 상태
- `ClientRPC_HideLoadingTransition`이 전송되지 않음
- `OnPawnReady`, `OnPawnHasName` 이후에도 `HandleLoadingComplete` 호출 안됨

#### 최종 해결 (성공) ✅ - AnnouncePlayerJoin에서 직접 로딩 숨김
**수정 위치**: `UChatPlayerSystem.cpp:117-131`
```cpp
void UChatPlayerSystem::AnnouncePlayerJoin()
{
    // "행궁에 이르셨습니다" 메시지 전송
...
**결과**: ✅ 정상 작동!
**장점**:
- 실제 플레이 가능한 시점(채팅 메시지 표시 가능)에 로딩 숨김
- 복잡한 로딩 시스템 흐름과 독립적
- 명확하고 직관적인 로직
- 호스트/게스트 모두 동일하게 작동
...
#### ✅ 정상 동작 시 게스트 로그 (최종 해결 후)
```
[LOADING_FLOW] ClientRPC_ShowLoadingTransition received - showing loading screen
[LOADING_FLOW] OnPawnReady called for BP_PlayerActor_C_0
[LOADING_FLOW] OnPawnHasName called
[LOADING_FLOW] AnnouncePlayerJoin - Player entered game, hiding loading screen
LogCoffee: [플레이어명] 전하 어가, 행궁에 이르셨습니다.
[로딩 위젯 사라짐]
```
...
[LOADING_FLOW] OnPawnReady called for BP_PlayerActor_C_0
[LOADING_FLOW] OnPawnHasName called
[여기서 멈춤 - HandleLoadingComplete 호출 안됨]
[로딩 위젯이 계속 표시됨 ❌]
```
...
**문서 버전**: 2.0
**최초 작성일**: 2025-11-09
**최종 수정일**: 2025-11-09 (AnnouncePlayerJoin에서 직접 로딩 숨김)
**관련 이슈**: #게스트_로딩_위젯_미제거
**상태**: ✅ 해결 완료

**변경 이력**:
- v1.0 (2025-11-09 14:42): 초기 분석 및 PostLoadMapWithWorld 수정 방안 제시
- v1.1 (2025-11-09 14:52): `Broadcast_HideLoading()` 호출 추가 (실패 - 델리게이트 미호출)
- v1.2 (2025-11-09 14:53): `ForceCompleteForGuest()` 함수 추가 (실패 - HandleLoadingComplete 미호출)
- v2.0 (2025-11-09 15:03): ✅ **최종 해결** - `AnnouncePlayerJoin()`에서 직접 로딩 화면 숨김

**참조 파일**:
- ✅ `Source/YiSan/Character/Private/UChatPlayerSystem.cpp` (Line 117-131) - **최종 해결**
- `Source/YiSan/Loading/Private/UYiSanLoading.cpp` (실패한 시도)
- `Source/YiSan/Character/Private/APlayerControl.cpp` (실패한 시도)
